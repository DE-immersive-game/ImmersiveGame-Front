...
const LoaderBar: React.FC<ProgressBarProps> = ({ scoreA, scoreB }) => {
  ...
  return (
    <div className="relative h-[50px] bg-gray-800 rounded-full ">
      {/* Barre de l'équipe A */}
      <div
        className="absolute h-full bg-natural-gradient  shadow-[0px_0px_43px_1px_#7FFD50]  rounded-l-full "
        style={{ width: `${APercentage}%` }}
      >
        <div className="h-full w-full overflow-hidden relative  rounded-l-full ">
      ...
      {/* Barre de l'équipe B */}
      <div
        className="absolute h-full bg-futuristic-gradient right-0   shadow-[0px_0px_43px_1px_#7757FF]  rounded-r-full "
        style={{ width: `${BPercentage}%` }}
      >
        <div className="h-full w-full overflow-hidden relative  rounded-r-full ">
